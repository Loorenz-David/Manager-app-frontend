  ...
  return [
    {
      ...
      task: {
        ...
        item: {
          ...
          sku: "SKU-NEW-001",
          item_image: {
            client_id: "img_item_1",
            image_url: "https://example.com/item-1.webp",
            width_px: null,
            height_px: null,
    ...
    },
    {
      ...
      task: {
        ...
        item: {
          ...
          sku: "SKU-ACT-002",
          item_image: {
            client_id: "img_item_2",
            image_url: "https://example.com/item-2.webp",
            width_px: null,
            height_px: null,
            file_size_bytes: 32_000,
          },
    ...
    },
    {
      ...
      task: {
        ...
        item: {
          ...
          sku: "SKU-RES-003",
          item_image: {
            client_id: "img_item_3",
            image_url: "https://example.com/item-3.webp",
            width_px: null,
            height_px: null,
            file_size_bytes: 32_000,
          },
...
async function installCasesMocks(
  ...
  const taskDetails = {
    ...
    task_2: createTaskDetail(
      "task_2",
      "ART-DETAIL-002",
      "internal",
      "store_return",
    ),
    task_3: createTaskDetail(
      "task_3",
      "ART-DETAIL-003",
      "pre_order",
      "before_purchase",
  ...
  await page.route("**/api/v1/cases/*/links", async (route) => {
    const caseId = new URL(route.request().url()).pathname
      .split("/")
      .at(-2) as keyof typeof caseLinks;
  ...
  await page.route("**/api/v1/cases/*/state", async (route) => {
    const caseId = new URL(route.request().url()).pathname
      .split("/")
      .at(-2) as keyof typeof caseDetails;
    ...
    if (
      ...
      caseDetails[caseId].initial.case.state = body.new_state as
        | "open"
        | "resolving"
        | "resolved";
  ...
  await page.route("**/api/v1/cases/*/participants", async (route) => {
    const caseId = new URL(route.request().url()).pathname
      .split("/")
      .at(-2) as keyof typeof caseParticipants;
  ...
  await page.route("**/api/v1/cases/messages/mark-read", async (route) => {
    ...
    const caseId = participantToCaseId[
      body.case_participant_client_id
    ] as keyof typeof caseDetails;
      ...
      body: JSON.stringify({
        ...
        data: {
          last_read_message_seq:
            participant?.last_read_message_seq ?? body.up_to_message_seq,
        },
      }),
    });
  });
  ...
  await page.route("**/api/v1/cases/*", async (route) => {
    ...
    const beforeMessageSeq = beforeMessageSeqParam
      ? Number(beforeMessageSeqParam)
      : null;
  ...
  await page.route("**/api/v1/tasks/*", async (route) => {
    const taskId = new URL(route.request().url()).pathname
      .split("/")
      .at(-1) as keyof typeof taskDetails;

    if (options?.unavailableTaskIds?.includes(taskId)) {
      await route.fulfill({
        status: 404,
        contentType: "application/json",
        body: JSON.stringify({
...
test.describe("cases page", () => {
  test("renders groups, filters client-side, shows unread badge, and opens the conversation shell", async ({
    ...
    await expect(page).toHaveURL(/\/cases\/case_new_open$/);

    await expect(page.getByTestId("case-conversation-header")).toBeVisible();
    await expect(
      page.getByTestId("case-conversation-context-banner"),
    ).toBeVisible();
    await expect(page.getByTestId("case-message-list")).toBeVisible();
    await expect(
      page.getByTestId("case-message-row-ccm_case_new_open_13"),
    ).toBeVisible();
    await expect(
      page.getByTestId("case-conversation-context-banner"),
    ...
    await expect(
      page.getByTestId("case-conversation-primary-label"),
    ).toHaveText("ART-DETAIL-001");
    await expect(page.getByTestId("case-conversation-subtitle")).toHaveText(
  ...
  test("renders cases when the list response omits task snapshots", async ({
    page,
  }) => {
    await installCasesListWithoutTaskMocks(page);
  ...
  test("renders message separators, own and other bubbles, avatars, and deleted placeholders", async ({
    ...
    await page
      .getByTestId("case-conversation-scroll-container")
      .evaluate((element) => {
        element.scrollTop = 0;
      });
    await expect(
      page.getByTestId("case-message-date-separator-2026-05-25"),
    ).toBeVisible();
    await expect(
      page.getByTestId("case-message-row-ccm_case_new_open_7"),
    ).toHaveAttribute("data-own-message", "true");
    await expect(
      page.getByTestId("case-message-row-ccm_case_new_open_8"),
    ).toHaveAttribute("data-own-message", "false");
    await expect(
      page.getByTestId("case-message-avatar-ccm_case_new_open_8"),
    ).toBeVisible();
    await expect(
  ...
  test("conversation still renders when optional task context fails to load", async ({
    ...
    await expect(
      page.getByText("Case conversation could not be loaded."),
    ).toHaveCount(0);
    await expect(
      page.getByTestId("case-conversation-primary-label"),
    ).toHaveText("CARD-NEW-001");
    await expect(page.getByTestId("case-conversation-subtitle")).toHaveText(
      "Return • After purchase",
    );
    await expect(
  ...
  test("scrolling to the top loads older messages and preserves the visible anchor", async ({
    ...
    const anchorBefore = await anchor.evaluate(
    ...
    expect(
      detailRequests.some((request) => request.caseId === "case_new_open"),
    ).toBe(true);
    expect(
      detailRequests.some(
        (request) =>
  ...
  test("opening a conversation marks the latest visible message as read and clears the cases unread badge without duplicate calls", async ({
    ...
    await expect(
      page.getByTestId("case-message-row-ccm_case_new_open_6"),
    ).toBeVisible();
    ...
    await expect(page.getByTestId("case-conversation-slide")).not.toBeVisible();
    await expect(
      page.getByTestId("case-card-unread-case_new_open"),
    ).toHaveCount(0);
  });
  ...
  test("info button opens the task info sheet and the task detail slide", async ({
    ...
    await expect(page.getByTestId("case-task-info-state")).toContainText(
      "Assigned",
    );
    await expect(page.getByTestId("case-task-info-card")).toContainText(
      "ART-DETAIL-001",
    );
    await expect(page.getByTestId("case-task-info-card")).toContainText(
      "Return",
    ...
    const taskDetailFollowsConversation = await page.evaluate(() => {
      ...
      const taskDetail = document.querySelector(
  ...
  test("state button maps backend transitions and closes the slide on success", async ({
    ...
    await expect(
      page.getByTestId("case-conversation-primary-label"),
    ).toHaveText("ART-DETAIL-003");
    await expect(page.getByTestId("case-conversation-subtitle")).toHaveText(
      "Pre-order • Before purchase",
    );
    await expect(page.getByTestId("case-conversation-state-button")).toHaveText(
      "Resolve",
    );
