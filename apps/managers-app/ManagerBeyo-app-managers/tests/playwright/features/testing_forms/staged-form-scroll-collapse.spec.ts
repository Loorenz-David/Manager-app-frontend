...
test.describe('Staged form scroll collapse', () => {
  ...
  test('timeline collapses after scrolling past 56 px and stays stable', async ({ page }) => {
    ...
    await expect(timeline).toHaveAttribute('data-compact', 'true', { timeout: 1000 });
  ...
  test('timeline re-expands after scrolling back near the top', async ({ page }) => {
    const scrollContainer = page.getByTestId('staged-form-scroll-container');
    const timeline = page.getByTestId('staged-form-timeline');
    ...
    await expect(timeline).toHaveAttribute('data-compact', 'false', { timeout: 1000 });
  });

  test('can scroll to the bottom of a short step without oscillation', async ({ page }) => {
    const scrollContainer = page.getByTestId('staged-form-scroll-container');
    const timeline = page.getByTestId('staged-form-timeline');
    ...
    if (scrollHeight <= clientHeight) {
      test.skip(true, 'Content is not scrollable on this step — test not applicable');
      return;
    }
    ...
    await page.waitForTimeout(600);
    await expect(timeline).toHaveAttribute('data-compact', compactAfterScroll ?? 'false');
  });

  test('navigating to a new step resets scroll and expands timeline instantly', async ({ page }) => {
    const scrollContainer = page.getByTestId('staged-form-scroll-container');
    const timeline = page.getByTestId('staged-form-timeline');
