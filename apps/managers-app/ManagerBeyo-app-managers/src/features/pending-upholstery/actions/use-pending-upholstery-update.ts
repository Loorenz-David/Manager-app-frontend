...
type PendingUpdateInput = UpdateItemUpholsteryInput & {
  taskId: string;
};
...
export function usePendingUpholsteryUpdate(itemId: string | null) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ taskId: _taskId, ...input }: PendingUpdateInput) =>
      updateItemUpholstery(input),
