...
import {
  fetchUpdateUpholsteryListOrder,
} from '../api/fetch-update-upholstery-list-order';
  ...
  const mutation = useMutation({
    ...
    onMutate: async ({ client_id, list_order }): Promise<UpdateListOrderContext> => {
      ...
      await queryClient.cancelQueries({
        queryKey: upholsteryKeys.detail(client_id as UpholsteryId),
      });

      const previousLists = queryClient.getQueriesData<PickerListData>({
        queryKey: upholsteryKeys.pickerLists(),
      });
      const detailKey = upholsteryKeys.detail(client_id as UpholsteryId);
    ...
    onError: (_error, variables, context) => {
      ...
      queryClient.setQueryData(
        upholsteryKeys.detail(variables.client_id as UpholsteryId),
    ...
    onSuccess: (updatedUpholstery) => {
      ...
      queryClient.setQueryData(
        upholsteryKeys.detail(updatedUpholstery.client_id as UpholsteryId),
        updatedUpholstery,
      );
    },
    onSettled: (_data, _error, variables) => {
      ...
      if (variables) {
        void queryClient.invalidateQueries({
          queryKey: upholsteryKeys.detail(variables.client_id as UpholsteryId),
        });
      }
    },
  });
