...
import { useItemCategoriesPickerQuery } from '../api/use-item-categories-picker';
...
export function useItemCategoryPickerFlow() {
  ...
  const byMajorCategory = resolvedOptions.reduce<Record<string, typeof resolvedOptions>>(
    (acc, option) => {
  ...
  return {
    ...
    byMajorCategory,
    isLoading: isPending && options.length === 0,
  };
}
