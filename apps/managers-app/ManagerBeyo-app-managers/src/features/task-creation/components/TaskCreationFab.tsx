...
const FAB_POSITION_CLASS = 'bottom-[calc(var(--safe-bottom,0px)+0.75rem)] right-4';
    ...
    <>
      {ACTION_BUTTONS.map((action, index) => (
        <motion.button
          ...
          className={cn(
            `fixed ${FAB_POSITION_CLASS} z-40 flex size-14 items-center justify-center rounded-full bg-primary text-card shadow-lg`,
          ...
          transition={{
            ...FAB_TRANSITION,
            delay: isOpen ? index * 0.03 : (ACTION_BUTTONS.length - 1 - index) * 0.03,
          }}
          animate={isOpen ? { scale: 0.7, x: action.x, y: action.y } : { scale: 0, x: 0, y: 0 }}
          type="button"
      ...
      <motion.button
        ...
        className={`fixed ${FAB_POSITION_CLASS} z-40 flex size-14 items-center justify-center rounded-full bg-primary text-card shadow-lg`}
