...
describe('useDeleteImage', () => {
  ...
  it('optimistically removes the image from every list and clears the detail query', async () => {
    ...
    expect(
      queryClient.getQueryData<ReturnType<typeof buildEntityImage>[]>(itemKey)?.map(
        (entry) => entry.image.client_id,
      ),
    ).toEqual(['img_2']);
    expect(queryClient.getQueryData<ReturnType<typeof buildEntityImage>[]>(caseKey)).toEqual([]);

    await waitFor(() => {
      expect(removeQueriesSpy).toHaveBeenCalledWith({ queryKey: imageKeys.detail('img_1') });
      expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: imageKeys.lists() });
    });
  });
  ...
  it('rolls back all lists on error', async () => {
    ...
    await expect(result.current.deleteImageAsync('img_1')).rejects.toThrow('delete failed');
    ...
    expect(
      queryClient.getQueryData<ReturnType<typeof buildEntityImage>[]>(caseKey)?.map(
        (entry) => entry.image.client_id,
      ),
    ).toEqual(['img_1']);
  });
