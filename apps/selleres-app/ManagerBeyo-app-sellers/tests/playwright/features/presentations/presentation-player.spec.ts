...
function createAccessToken(): string {
  ...
  return `${encode({ alg: "none", typ: "JWT" })}.${encode({
    user_id: "usr_seller_presentations",
    username: "Seller",
    workspace_id: "ws_presentations",
    workspace_name: "Presentation Workspace",
    workspace_role_id: "wrole_seller_presentations",
    role_name: "seller",
    workspace_role_name: "seller",
...
function presentation(
  ...
  return {
    client_id: `aup_seller_${suffix}`,
    logical_client_id: `aup_seller_${suffix}`,
    version: 1,
    title: `Seller ${suffix}`,
    summary: null,
  ...
  await page.route("**/api/v1/users/me", (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          user: {
            client_id: "usr_seller_presentations",
            email: "seller-presentations@example.com",
            username: "Seller",
          },
        },
      }),
...
test("presentation-player defers off-home and completes the seller flow", async ({
  page,
}) => {
  ...
  expect(terminalActions).toEqual([
    { id: "aup_seller_modal", action: "dismissed" },
    { id: "aup_seller_full_screen", action: "completed" },
    { id: "aup_seller_slide_page", action: "completed" },
  ]);
