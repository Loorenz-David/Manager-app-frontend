...
import type { DeleteImageInput } from "../api/delete-image";
...
export function useDeleteImage() {
    ...
    onMutate: async ({ imageClientId }): Promise<DeleteImageContext> => {
      ...
      queryClient.setQueriesData<EntityImage[]>(
        ...
        (old) => {
          return (
            old?.filter(
              (entityImage) => entityImage.image.client_id !== imageClientId,
            ) ?? []
          );
        },
      );
  ...
  const deleteImageWithOptions = (input: DeleteImageInput) =>
    mutation.mutate(input);
  const deleteImageWithOptionsAsync = (input: DeleteImageInput) =>
    mutation.mutateAsync(input);
  const deleteImageDefault = (imageClientId: string) =>
    mutation.mutate({ imageClientId });
  const deleteImageDefaultAsync = (imageClientId: string) =>
    mutation.mutateAsync({ imageClientId });

  return {
    deleteImage: deleteImageDefault,
    deleteImageAsync: deleteImageDefaultAsync,
    deleteImageWithOptions,
