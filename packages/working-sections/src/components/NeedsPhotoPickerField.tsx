...
import { useNeedsPhotoPickerFlow } from "../flows/use-needs-photo-picker.flow";

export function NeedsPhotoPickerField(): React.JSX.Element {
  const { control } = useFormContext();
  const flow = useNeedsPhotoPickerFlow();
  const { field, fieldState } = useController({
    name: "needs_photo_assignment",
    control,
    defaultValue: null,
  });
  ...
  return (
    <div className="flex flex-col gap-1.5" data-testid="needs-photo-picker-field">
      <div
        ...
        data-testid="needs-photo-picker-card"
        role="button"
        tabIndex={0}
        onClick={handlePress}
        onKeyDown={(event) => {
      ...
      >
        ...
        <span className="flex min-w-0 flex-1 flex-col gap-1">
          <span className="truncate text-sm font-medium">Needs photo</span>
          <span
            className={cn(
      ...
      <FieldErrorPill
        data-testid="needs-photo-picker-error"
