...
export const NumberInput = forwardRef<HTMLInputElement, NumberInputProps>(
  (
    ...
    const [draft, setDraft] = useState(() =>
      formatNumberDisplay(externalValue),
    );
    const lastEmittedValueRef = useRef<
      number | null | typeof INTERNAL_VALUE_UNSET
    ...
    function emitValue(
      ...
      nextMeta: NumberInputChangeMeta,
    ): void {
    ...
    function handleDraftChange(nextRawValue: string): void {
      const sanitizedDraft = sanitizeNumberInput(nextRawValue, {
        allowDecimal,
      });
      ...
      emitValue(
        nextMeta.hasParsedValue ? nextMeta.parsedValue : null,
        nextMeta,
      );
    }
    ...
    function normalizeDraft(): void {
      ...
      const normalizedMeta = parseNumberDraft(normalizedDraft, {
        allowDecimal,
      });
    ...
    function handleStep(direction: 1 | -1): void {
      const baseValue = draftMeta.hasParsedValue
        ? draftMeta.parsedValue
        : externalValue;
    ...
    const currentValue = draftMeta.hasParsedValue
      ? draftMeta.parsedValue
      : externalValue;
    ...
    const decrementDisabled =
      disabled ||
      (currentValue !== null &&
        Object.is(decrementTarget, clampNumber(currentValue, min, max))) ||
      (currentValue === null &&
        min !== undefined &&
        min !== null &&
    ...
    const incrementDisabled =
      disabled ||
      (currentValue !== null &&
        Object.is(incrementTarget, clampNumber(currentValue, min, max)));
