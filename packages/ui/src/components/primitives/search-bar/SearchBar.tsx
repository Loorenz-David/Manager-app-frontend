...
function FilterCountBadge({ count }: { count: number }): React.JSX.Element {
  return (
    <m.span
      ...
      className="inline-flex min-w-4 items-center justify-center rounded-full bg-primary p-1 text-[10px] font-semibold leading-none text-card"
    ...
    return (
      ...
      >
        ...
        {isLoading ? (
          <span
            className={cn(
              "flex h-full shrink-0 items-center justify-center text-icon",
            ...
            {showFilterButton ? (
              ...
              >
                ...
                <AnimatePresence initial={false}>
                  {isFilterActive ? (
                    <FilterCountBadge key="badge" count={activeFilterCount} />
                  ) : null}
                </AnimatePresence>
              </button>
            ) : null}
