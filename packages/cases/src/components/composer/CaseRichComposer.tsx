...
export function CaseRichComposer({
  ...
  const shouldHideScrollToBottomCta =
    isEditorFocused || controller.draftAttachmentCount > 0;
  ...
  useEffect(() => {
    onToolbarVisibilityChange?.(shouldHideScrollToBottomCta);
  }, [onToolbarVisibilityChange, shouldHideScrollToBottomCta]);
    ...
    >
      ...
      >
        <CaseComposerDraftImagesProvider
          caseClientId={controller.caseDetail?.case.client_id ?? null}
          draftMessageClientId={controller.draftMessageClientId}
        >
          <CaseComposerAttachmentStrip
            hidden={isEditing || !controller.caseDetail}
          ...
          <div className="rounded-[1.9rem] border border-border bg-card px-2 py-2 shadow-[0_10px_24px_rgba(0,0,0,0.08)]">
            <div className="flex items-end gap-2">
              {!isEditing ? <CaseComposerInlineCameraButton /> : null}
                ...
                >
                  <LazyCaseComposerEditor
                    ...
                    onChange={({ content, plainText }) => {
                      if (isEditing) {
                        controller.setEditingComposerContent(
                          content,
                          plainText,
                        );
                        return;
