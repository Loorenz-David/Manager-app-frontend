...
import { ManagedPhoneInput } from "../../../../phone-input/src/components/ManagedPhoneInput";

export function CustomerPhoneField(): React.JSX.Element {
  const {
    control,
    formState: { errors },
  } = useFormContext();
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <ManagedPhoneInput
        ...
        value={field.value ?? ""}
        onValueChange={(nextValue: string) => field.onChange(nextValue)}
      />
    </div>
  );
}
