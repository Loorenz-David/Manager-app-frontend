...
| `src/lib/text-box-layout.ts` | Text box geometry: `textBoxHeightFraction` (height of the text wrapped at the authored width), `isHuggingTextHeight` (auto-height detection — the box describes itself, there is no stored flag), `withMeasuredTextHeight(previous, next)` | Auto-height rules. Hug detection must read the **pre-edit** element, or a font-size change reads as "the author fixed the height". |
| `src/lib/slide-duration.ts` | Slide duration rules: `roundSlideDurationMs` (2 s floor, 500 ms step, **no ceiling**), `formatSlideDuration` (`4.0s` / `1:30`), `parseSlideDuration` (`8`, `8s`, `90`, `1:30`, `2m`, `500ms`), `SLIDE_DURATION_SLIDER_MAX_SECONDS = 12` | Duration bounds. The backend has no maximum (`validate_slide_timeline` only requires `> 0`); the slider's 2–12 s is the **handle range**, not a cap — longer values are typed into the value field. |
