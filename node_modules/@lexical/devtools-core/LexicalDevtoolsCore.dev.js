...
function registerLexicalCommandLogger(editor, setLoggedCommands) {
  const unregisterCommandListeners = [];
  let index = 0;
  for (const command of editor._commands.keys()) {
    unregisterCommandListeners.push(editor.registerCommand(command, payload => {
      index += 1;
      const entry = {
        index,
        payload,
...
function useLexicalCommandsLog(editor) {
  ...
  react.useEffect(() => registerLexicalCommandLogger(editor, setLoggedCommands), [editor]);
