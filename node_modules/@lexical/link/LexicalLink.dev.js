...
class LinkNode extends lexical.ElementNode {
  ...
  insertNewAfter(_, restoreSelection = true) {
    const linkNode = lexical.$copyNode(this);
...
function $saveCaretPair(point) {
  const next = lexical.$caretFromPoint(point, 'next');
  return [next, next.getFlipped()];
}
function $restoreCaretPair(point, pair) {
  for (const caret of pair) {
    if (caret.origin.isAttached()) {
      const normalized = lexical.$normalizeCaret(caret);
      lexical.$setPointFromCaret(point, normalized);
      return;
    }
  }
}
...
function $linkNodeTransform(link) {
  ...
  function $restoreSelection() {
    if (lexical.$isRangeSelection(selection)) {
      ...
      lexical.$normalizeSelection__EXPERIMENTAL(selection);
    }
  }
  let transformed = false;
  for (const caret of lexical.$getChildCaret(link, 'next')) {
    const node = caret.origin;
    if (lexical.$isElementNode(node) && !node.isInline()) {
      const blockChildren = node.getChildren();
      if (blockChildren.length > 0) {
        const innerLink = lexical.$copyNode(link);
        innerLink.append(...blockChildren);
        node.append(innerLink);
        transformed = true;
      }
      utils.$insertNodeToNearestRootAtCaret(node, lexical.$rewindSiblingCaret(caret), {
        $shouldSplit: () => false
      });
    }
  }
  ...
  function $fixMergeBoundaryCaret(pair, absorbingLink, mergingLink) {
    ...
    const $isAffected = caret => lexical.$isSiblingCaret(caret) && caret.origin.is(absorbingLink);
    ...
    const fixed = lexical.$normalizeCaret(lexical.$getChildCaret(mergingLink, 'next'));
...
function $splitLinkAtSelection(parentLink, extractedNodes) {
  ...
  const isExtractedChild = child => extractedKeys.has(child.getKey()) || lexical.$isElementNode(child) && extractedNodes.some(n => parentLink.isParentOf(n) && child.isParentOf(n));
  ...
  if (isAtStart) {
    ...
    if (trailingChildren.length > 0) {
      const newLink = lexical.$copyNode(parentLink);
...
function registerLink(editor, stores) {
  return utils.mergeRegister(editor.registerNodeTransform(LinkNode, $linkNodeTransform), editor.registerCommand(TOGGLE_LINK_COMMAND, payload => {
  ...
  }, lexical.COMMAND_PRIORITY_EDITOR), extension.effect(() => {
