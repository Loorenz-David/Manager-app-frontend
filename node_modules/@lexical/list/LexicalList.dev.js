...
class ListNode extends lexical.ElementNode {
  ...
  splice(start, deleteCount, nodesToInsert) {
    ...
    const $newListItem = exampleListItem ? () => lexical.$copyNode(exampleListItem) : $createListItemNode;
    ...
    for (let i = 0; i < nodesToInsert.length; i++) {
      ...
      if (!$isListItemNode(node)) {
        ...
        listItemNodesToInsert[i] = $newListItem().append(lexical.$isElementNode(node) && !($isListNode(node) || node.isInline()) ? lexical.$createTextNode(node.getTextContent()) : node);
      }
    }
...
function registerList(editor, options) {
  ...
  }, lexical.COMMAND_PRIORITY_LOW), editor.registerNodeTransform(ListItemNode, node => {
