...
import { $isDecoratorNode, $getNodeByKey, $isLineBreakNode, $isTextNode, $getSelection, $isRangeSelection, $isElementNode, $createChildrenArray, $getNodeByKeyOrThrow, removeFromParent, createEditor, $isRootNode, $getWritableNodeState, $getRoot, RootNode, ElementNode, TextNode, $getState, createState, setDOMStyleObject, COLLABORATION_TAG, HISTORIC_TAG, $addUpdateTag, SKIP_SCROLL_INTO_VIEW_TAG, $createParagraphNode, createCommand } from 'lexical';
import { XmlText, XmlElement, Map as Map$1, Doc, typeListToArraySnapshot, Snapshot, isDeleted, XmlHook, ContentString, ContentFormat, emptySnapshot, PermanentUserData, iterateDeletedStructs, snapshot, createAbsolutePositionFromRelativePosition, createRelativePositionFromTypeIndex, compareRelativePositions, Item, YMapEvent, YTextEvent, YXmlEvent, UndoManager } from 'yjs';
    ...
    if (clientID !== 0 && clientID !== localClientID) {
      ...
      if (focusing) {
        ...
        if (anchorKey !== null && focusKey !== null) {
          ...
          if (selection === null) {
            selection = createCursorSelection(cursor, anchorKey, anchorOffset, focusKey, focusOffset, collabTheme);
          } else {
            const anchor = selection.anchor;
            const focus = selection.focus;
