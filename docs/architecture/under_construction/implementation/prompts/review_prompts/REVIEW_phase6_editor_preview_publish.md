# Review — Phase 6: Preview overlay, publish dialog (audience/metadata), lifecycle actions, read-only + new-version

Paste this whole prompt into a fresh Claude (Opus) review session.

---

You are the **implementation reviewer** for Phase 6 of the ManagerBeyo presentation capability, working in the `frontend/` monorepo root. The implementation was done by a Codex session from an approved plan; component styling was done by a separate Claude session (“kit” components — read-only for Codex). Your job: verify the implementation against its plan and the shared rules, then either bless it or produce a corrections plan. You change no code.

## What was implemented

- Phase plan: `docs/architecture/under_construction/implementation/PLAN_presentation_phase6_editor_preview_publish_20260722.md` — if not there, it was archived after validation: look in `docs/architecture/archives/implementation/`.
- Implementation summary (written by the Codex session): `docs/architecture/implemented_summaries/SUMMARY_presentation_phase6_editor_preview_publish_20260722.md`.
...
- V2 compliance: NO code anywhere writes `slide.title` from composition text (the dropped mitigation must not exist).
- Preview renders through the shared runtime renderer only; entry flushes dirty slides; dev-only parity assertion vs `GET /{id}/preview` stripped from prod.
- Publish dialog expresses every audience form in `06_admin_audience.md`; no workspace field; `selected_users_only` needs ≥1 user client-side.
- Explicit `display_priority` always sent on publish-time PATCH (PATCH does not re-derive from category).
- 422 causes map to specific dialog locations; raced 409 → friendly refetch state.
- Read-only: single `readOnly` flag disables every mutating control across Phase 4–5 surfaces; "Edit as new version" re-enters edit mode on the new id; dashboard grouping reflects both versions.
- The `new-version`-from-archived verification outcome is recorded in the Review log.
