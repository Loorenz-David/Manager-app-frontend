...
- `2026-07-23` Claude (builder), live matrix + close-out: **ALL GATES GREEN — criterion 9 recorded, criterion 10 executed.** Root typecheck green (user fixed their pause-reasons type error). During the live run a blocking consumer-schema bug surfaced and was fixed (logic-lane, recorded): `ConsumerPresentationSchema.category` was non-nullable while the backend serves `category: null` (nullable per `07_enums.md`) — the parse failed silently and the player never opened; fixed + regression test (`packages/presentations/src/types.test.ts`, 18/18 green). **Live matrix evidence** (operator-run servers: backend :8000, studio :5176, managers :5173/fayoz, workers :5174/roman, sellers :5175/stina): three announcements published targeting a single `app_key` each; every app auto-showed ONLY its own on the home route (12/12 scripted assertions): manager — timed deck auto-completed, closed, and never reappeared on cold reload; worker — non-dismissible showed NO dismiss affordance, acknowledge footer was the only exit, completed and never reappeared; seller — CTA button rendered and navigated in-app. **Realtime**: publishing while the manager app sat open on home auto-opened the player without reload (socket → invalidate → refetch → open). **Off-home deferral**: publishing while on the tasks tab did NOT auto-open; returning home released it. Screenshots + scripts archived in the session record; chrome check passed (kit-faithful player in all three apps). No frontend eligibility logic anywhere — the backend decided every routing outcome.
